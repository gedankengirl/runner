...
local startScale = script:GetCustomProperty("StartScale")
local endScale = script:GetCustomProperty("EndScale")
...
local setStartScale = script:GetCustomProperty("SetStartScale")
...
function Tick(deltaTime)
	...
	if progress == 0 and setStartScale == true then 
		script.parent:SetScale(startScale)
	end
	...
	if progress >= duration+startWaitTime and progress < dt then return end 
	
	if progress >= dt then
		if reset == true then
			progress = 0 
			if triggerNextAnim == true and scriptToTrigger ~= nil then 
				scriptToTrigger.context.Animate()
			end
		elseif reset == false then 
			isActive = false
			if triggerNextAnim == true then
				if scriptToTrigger ~= nil then 
					scriptToTrigger.context.Animate()
				end
			end				 
			return
	...
	script.parent:SetScale(Vector3.Lerp(startScale, endScale, t)) 
